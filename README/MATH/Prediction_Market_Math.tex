...
\begin{itemize}
    ...
    \item \textbf{Post-Bet Probability, Not Previous Probability:} For each bet, \(p_i\) is the probability immediately after that bet is applied. This prevents a bet from receiving course payout solely from the probability movement caused by that same bet. Subsequent probability movement changes the absolute-distance reward \(d_i = |R - p_i|\), regardless of direction. Course payouts are therefore outcome-neutral: a YES or NO bet can receive payout when the later market probability moves in either direction. The separate sellability rules determine when that resulting value may be sold.
\end{itemize}
